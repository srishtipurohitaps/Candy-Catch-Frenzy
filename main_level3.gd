...
var bomb_chance = 0.40
var spawn_wait_start = 0.5
var max_time = 40.0
var max_lives = 5
...
func _process(delta):
	time_pass += delta
	var hue = fmod(time_pass * 0.15, 1.0)
	background.color = Color.from_hsv(hue, 0.4, 0.95)
	if game_active and lives > 0:
		game_time -= delta
		timer_label.text = "Time: %d" % int(game_time)
		if game_time < 10:
			timer_label.modulate = Color(1, 0.3, 0.3) if fmod(time_pass, 0.5) < 0.25 else Color.WHITE
		if game_time <= 0:
			game_over()
	if shake_amount > 0:
...
func _on_spawn_timer_timeout():
	if not game_active:
		return
	spawn_timer.wait_time = max(0.2, spawn_wait_start - ((max_time - game_time) / 60.0))
	var r = randf()
	if r < bomb_chance:
		spawn_bomb()
	else:
...
func spawn_bomb():
	...
	bomb.fall_speed = 300 + (max_time - game_time) * 5.0
...
func catch_candy(points):
	...
	if combo_timer > 0:
		...
		combo_count = 1
	combo_timer = 3.0
	points = int(points * (1 + combo_count * 0.25))
	var old_score = score
	score += points
	...
	if combo_count > 1:
		...
		lbl.global_position = $Basket.global_position - Vector2(0,120)
		add_child(lbl)
		var tw = create_tween()
		tw.tween_property(lbl, "global_position:y", lbl.position.y - 60, 1.0)
		tw.parallel().tween_property(lbl, "modulate:a", 0, 1.0)
...
func activate_rainbow_powerup():
	lives = 3
	lives_label.text = "Lives: %d" % lives
	$Basket.scale = Vector2(1.6,1.6)
	var glow = create_tween().set_loops()
	glow.tween_property($Basket, "modulate", Color(1.5,1.5,1.5), 0.5)
	glow.tween_property($Basket, "modulate", Color.WHITE, 0.5)
	await get_tree().create_timer(5.0).timeout
...
func game_over():
	...
	tween.tween_property(game_over_panel, "scale", Vector2.ONE, 0.6)
