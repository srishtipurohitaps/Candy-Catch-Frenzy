...
# Level 3: Hard (very challenging)
var bomb_chance = 0.25
var spawn_wait_start = 0.6
var max_time = 40.0
...
func _on_spawn_timer_timeout():
	if not game_active:
		return
	spawn_timer.wait_time = max(0.2, spawn_wait_start - ((max_time - game_time) / 80.0))
...
func spawn_candy():
	...
	candy.fall_speed += (max_time - game_time) * 3.0
	add_child(candy)

func spawn_bomb():
	var bomb = bomb_scene.instantiate()
	bomb.position = Vector2(randf_range(100, 1052), -50)
	bomb.fall_speed = 280 + (max_time - game_time) * 4.0
	add_child(bomb)
...
func catch_candy(points):
	...
	if combo_timer > 0:
		...
		combo_count = 1
	combo_timer = 3.0
	points = int(points * (1 + combo_count * 0.2))
